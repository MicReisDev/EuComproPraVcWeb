...
async function fetchProducts() {
  try {
    const response = await axios.get(
      "/produtos?populate=*&pagination[pageSize]=100"
    );
    ...
    const products = cmsProducts.map((item) => {
      return {
        ...
        image: item.imagens ? item.imagens.map((img) => img.url) : [],
        batchUnits: item.quantidade_no_lote,
        shortDescription: item.descricao || "Sem descrição disponível.",
        otherInformations:
          item.outras_informacoes || "Sem informações adicionais disponíveis.",
